...
def test_end_to_end_pull_update_render(db_conn, sample_rows_extra):
    # Arrange: wire real DB insert + analysis with fake scraper data.
    def fake_scraper():
    ...
    # Act: pull data, update analysis, and render page.
    ...
    # Assert: analysis page includes formatted percentages and label.
...
def test_multiple_pulls_idempotent(db_conn, sample_rows_extra):
    # Arrange: prepare app for repeated pulls.
    def fake_scraper():
        return sample_rows_extra
    ...
    client = app.test_client()
    # Act: pull twice to exercise idempotency.
    response = client.post("/pull-data")
    assert response.status_code == 200
    response = client.post("/pull-data")
    # Assert: DB count equals unique URLs and page still renders.
    assert response.status_code == 200
